...
import sys
from tkinter.messagebox import NO  # Needed for passing argv to QApplication
...
class Main(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def __init__(self):
        ...
        self.model = srcnn.SrCNN().to(device)
        self.model.load_state_dict(torch.load(path_navarp_denoise_model, map_location=device))
        self.model.eval()
        ...
        #self.actionExportDenoised.triggered.connect(self.denosieAndExport)
    ...
    def openfile(self):
        ...
        if file_path:
            ...
            if not self.file_loaded_flag:  # connect only once with first file
                ...
                # For setting the analyer from entry 
            ...
            if isomap_name == 'isoscan':  # top panel
                ...
                isoscan = self.entry.isoscan(
                    ...
                    curvature=self.get_curvature(
                        self.qsb_top_curvature.value()),
                    kbins=kbins,
                    denoised=self.exportDialog.denoisecheckBox.isChecked()
                )
